"""Benchmark StandardScaler TileLang vs Triton.

Phase: 8
Usage:
    python benchmarks/tilelang/bench_standard_scaler.py --backend all
    modal run scripts/modal/bench_primitive.py --primitive standard_scaler
"""
...
def benchmark_standard_scaler(backend: str, N: int = 4096, D: int = 128,
                              n_iters: int = 100, warmup: int = 10) -> float:
    """Benchmark fit_transform. Returns average time in ms."""
    import torch
    ...
    if backend == "tilelang":
        ...
        fn = lambda: tilelang_standard_scaler_fit_transform(X)
    elif backend == "triton":
        from flashlib.primitives.standard_scaler.triton import (
            flash_standard_scaler_fit_transform as triton_fit_transform,
        )
        fn = lambda: triton_fit_transform(X)
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark StandardScaler fit_transform")
    ...
    for N, D in shapes:
        ...
        for backend in backends:
            try:
                t = benchmark_standard_scaler(backend, N=N, D=D,
