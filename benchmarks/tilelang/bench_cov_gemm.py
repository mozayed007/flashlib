"""Benchmark cov_gemm TileLang vs Triton.

Phase: 8
Usage:
    python benchmarks/tilelang/bench_cov_gemm.py --backend all
...
def benchmark_cov_gemm(backend: str, N: int = 4096, D: int = 128,
                       n_iters: int = 100, warmup: int = 10) -> float:
    """Benchmark X^T @ X. Returns average time in ms."""
    import torch

    X = torch.randn(N, D, device="cuda", dtype=torch.float32)

    if backend == "tilelang":
        from flashlib.linalg.cov_gemm.tilelang.cov_gemm import tilelang_cov_gemm
        fn = lambda: tilelang_cov_gemm(X)
    elif backend == "triton":
        from flashlib.linalg.cov_gemm.triton import cov_gemm as triton_cov_gemm
        fn = lambda: triton_cov_gemm(X)
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark cov_gemm (X^T @ X)")
    ...
    for N, D in shapes:
        ...
        for backend in backends:
            try:
                t = benchmark_cov_gemm(backend, N=N, D=D,
